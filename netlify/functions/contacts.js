...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'GET') {
      const res  = await supaFetch('/contacts?order=date_creation.desc');
    ...
    if (method === 'POST') {
      ...
      const res  = await supaFetch('/contacts', {
        method: 'POST',
    ...
    if (method === 'PUT') {
      ...
      const body = JSON.parse(event.body);
      const res  = await supaFetch('/contacts?id=eq.' + encodeURIComponent(id), {
        method: 'PATCH',
        body:   JSON.stringify(body),
      });
      const data = await res.json();
    ...
    if (method === 'DELETE') {
      if (!id) return respond({ error: 'id manquant' }, 400);
      await supaFetch('/contacts?id=eq.' + encodeURIComponent(id), { method: 'DELETE' });
